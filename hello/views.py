...
def getGMapsDistance(origin,dest,city):
    ...
    while timeact == -1:
        try:
            ...
            if(timenum>45):
                string = "https://maps.googleapis.com/maps/api/distancematrix/json?origins="+origin+" "+city+"&destinations="+destination+" "+city+"&mode=transit&key="+key
                data = requests.get(string).json()
                if(data['rows'][0]['elements'][0]['status']=="ZERO_RESULTS"):
                    ...
                    timetransit = 0        
                    if(len(time)==2):
                        timetransit = int(time[0])
                    else:
                        timetransit = (int(time[0])*60) + (int(time[2]))
                timeact = timetransit
            else:
                timeact = timenum
    ...
    for i in range(0,len(locs)):
        try:
            ...
            if(len(time)==2):
                ...
                timenum = (int(time[0])*60) + (int(time[2]))
            if(timenum>45):
                string = "https://maps.googleapis.com/maps/api/distancematrix/json?origins="+name+" "+userTrip.city+"&destinations="+locs[i].address+" "+userTrip.city+"&mode=transit&key="+key
                data = requests.get(string).json()
                if(data['rows'][0]['elements'][0]['status']=="ZERO_RESULTS"):
...
def index(request):
    if(request.method=='POST'):
        ...
        if(postData["type"]=="ILP"):
            ...
            fbids = Trips.objects.values_list('fbid',flat= True).distinct()
...
def entries(request):
    if(request.method=='POST'):
        try:
            ...
            elif(postData["type"]=="CheckRecords"):
                res = checkrecords(postData)
                #userentries = {"records": [[loc.city,loc.locid,loc.name,loc.title,loc.hashtag,loc.description,loc.imagelink,loc.time,loc.rating,loc.price,loc.book,loc.deposit,loc.acttype,loc.hours,loc.provider,loc.website,loc.address,loc.coordinates] for loc in locs]}
